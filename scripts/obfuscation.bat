...
for /d %%a in (%1-*) do (
	...
	for /f tokens^=2^ delims^=^" %%l in ('findstr /i /r /c:"compile" %%~fa\%%a.gradle') do (
		for /f "tokens=2 delims=:" %%s in ("%%l") do (
			if "%%s"=="nre" (
                echo ^<jar name="bin/ext/%%s.jar" /^> >> config.xml
            ) else (
...
echo ^<class template="class org.bouncycastle.*" /^> >> config.xml
echo ^<class template="class org.apache.*" /^> >> config.xml
echo ^<class template="class org.eclipse.*" /^> >> config.xml
echo ^<class template="class org.glassfish.*" /^> >> config.xml
echo ^<class template="class org.objectweb.*" /^> >> config.xml
echo ^<class template="class org.slf4j.*" /^> >> config.xml
echo ^<class template="class com.fasterxml.*" /^> >> config.xml
echo ^<class template="class com.google.*" /^> >> config.xml
echo ^<class template="class com.kenai.*" /^> >> config.xml
echo ^<class template="class com.microsoft.*" /^> >> config.xml
echo ^<class template="class javax.json.*" /^> >> config.xml
echo ^<class template="class jni.*" /^> >> config.xml
echo ^<class template="class jnr.*" /^> >> config.xml
echo ^</ignore-classes^> >> config.xml
...
java -Xms256m -Xmx1024m -jar %NIAGARA_HOME%\..\.btib\lib\Allatori\lib\allatori.jar "config.xml"
